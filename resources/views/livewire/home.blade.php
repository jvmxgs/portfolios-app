<div class="flex flex-col gap-10">
    <h1 class="text-2xl text-dark-blue font-semibold">Community projects</h1>
        ...
        @forelse ($projects as $project)
            <article class="group w-full h-96 rounded-lg shadow-lg bg-cover" style="background-image: url('{{ $project->getFirstMediaUrl('images') }}')">
                <section class="w-full h-full flex flex-col gap-4 justify-end p-8 bg-gradient-to-t from-white">
                    ...
                    <p class="rounded-md bg-clip-text text-dark-blue font-bold [text-shadow:_0_1px_0_rgb(255_255_255_/_40%)]">{{ $project->title }} by <span class="text-bittersweet">{{ $project->user->name }}</span></p>
                    <div class="flex justify-end gap-3">
                        <button @click="$dispatch('toogleLike', {id: {{ $project->id }} })"><x-icon class="cursor-pointer w-6 h-6 text-manatee {{ $project->isLiked ? 'fill-bittersweet animate-heartbeat' : 'text-manatee' }}" name="heart" variant="solid" /></button>
                    </div>
                </section>
            </article>
        @empty
            <span class="text-manatee text-sm self-center">At the moment there is no project</span>
        @endforelse
    </section>
